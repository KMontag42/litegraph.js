  ...
  var LiteGraph = (global.LiteGraph = {
	...
	*/
    ...
    registerNodeType: function(type, base_class) {
      ...
      //extend class
      if (
        base_class.prototype //is a class
      )
	...
	*/
    ...
    getNodeTypesInCategory: function(category) {
      ...
      for (var i in this.registered_node_types)
        if (category == "") {
          if (this.registered_node_types[i].category == null) r.push(this.registered_node_types[i]);
    ...
    if (do_not_catch_errors) {
      ...
      for (var i = 0; i < num; i++) {
        for (var j = 0, l = nodes.length; j < l; ++j) {
          var node = nodes[j];
          if (node.mode == LiteGraph.ALWAYS && node.onExecute) node.onExecute();
        }
...
*/
  ...
  LGraph.prototype.computeExecutionOrder = function(only_onExecute) {
    ...
    for (var i = 0, l = this._nodes.length; i < l; ++i) {
      ...
      if (
        num == 0 //is a starting node
      )
        S.push(n); //num of input links
      else remaining_links[n.id] = num;
    }
...
*/
  ...
  LGraphNode.prototype.serialize = function() {
    ...
    if (this.outputs) for (var i = 0; i < this.outputs.length; i++) delete this.outputs[i]._data;
  ...
  LGraphNode.prototype.clone = function() {
    ...
    //remove links
    if (data.inputs) for (var i = 0; i < data.inputs.length; ++i) data.inputs[i].link = null;
...
*/
  LGraphNode.prototype.trigger = function(action, param) {
    ...
    for (var i = 0; i < this.outputs.length; ++i) {
      var output = this.outputs[i];
      if (output.type !== LiteGraph.EVENT || (action && output.name != action)) continue;
...
*/
  LGraphNode.prototype.getBounding = function() {
    return new Float32Array([
      ...
      this.pos[0] + this.size[0] + 4,
      this.pos[1] + this.size[1] + LGraph.NODE_TITLE_HEIGHT
    ]);
  };
    ...
    if (this.flags.collapsed) {
      ...
      if (
        isInsideRectangle(
          ...
          LiteGraph.NODE_COLLAPSED_WIDTH + 2 * margin,
        ...
        if (link_info.target_id == target_node.id) {
          ...
          if (target_node.onConnectionsChange)
            target_node.onConnectionsChange(
              ...
              link_info,
              input
            ); //link_info hasnt been modified so its ok
          if (this.onConnectionsChange)
...
**/
  LGraphNode.prototype.getConnectionPos = function(is_input, slot_number) {
    if (this.flags.collapsed) {
      if (is_input) return [this.pos[0], this.pos[1] - LiteGraph.NODE_TITLE_HEIGHT * 0.5];
      else
        return [
          this.pos[0] + LiteGraph.NODE_COLLAPSED_WIDTH,
          this.pos[1] - LiteGraph.NODE_TITLE_HEIGHT * 0.5
        ];
    ...
    if (is_input && this.inputs.length > slot_number && this.inputs[slot_number].pos)
      return [
        this.pos[0] + this.inputs[slot_number].pos[0],
        this.pos[1] + this.inputs[slot_number].pos[1]
      ];
    else if (!is_input && this.outputs.length > slot_number && this.outputs[slot_number].pos)
      return [
        this.pos[0] + this.outputs[slot_number].pos[0],
        this.pos[1] + this.outputs[slot_number].pos[1]
      ];
...
*/
  ...
  LGraphCanvas.prototype.processMouseDown = function(e) {
    ...
    if (e.which == 1) {
      //left button mouse
      if (!e.shiftKey) {
        //REFACTOR: integrate with function
          ...
          if (
            ...
            if (slot != -1) {
              ...
              if (this.connecting_output.type == LiteGraph.EVENT)
                ...
                input &&
                !input.link &&
                input.type == this.connecting_output.type //toLowerCase missing
              )
  ...
  LGraphCanvas.prototype.drawFrontCanvas = function() {
    ...
    if (this.graph) {
      ...
      //connections ontop?
      if (this.graph.config.links_ontop) if (!this.live_mode) this.drawConnections(ctx);
      ...
      if (this.connecting_pos != null) {
        ...
        else ctx.arc(this.connecting_pos[0], this.connecting_pos[1], 4, 0, Math.PI * 2);
        ctx.fill();
    ...
    if (
      ctx.finish2D //this is a function I use in webgl renderer
    )
      ctx.finish2D();
    ...
    if (!node.flags.collapsed) {
      ...
      if (node.inputs)
        for (var i = 0; i < node.inputs.length; i++) {
          ...
          if (slot.type === LiteGraph.EVENT) ctx.rect(pos[0] - 6 + 0.5, pos[1] - 5 + 0.5, 14, 10);
  ...
  LGraphCanvas.prototype.drawNodeShape = function(
    ...
    if (shape == "box") {
      ...
      if (selected) {
        ...
        ctx.strokeRect(
          -0.5,
          no_title ? -0.5 : -title_height + -0.5,
          size[0] + 2,
          no_title ? size[1] + 2 : size[1] + title_height + 2 - 1
        );
    ...
    if (!no_title) {
      ...
      else ctx.rect(3, -title_height + 3, title_height - 6, title_height - 6);
      ctx.fill();
  ...
  LGraphCanvas.prototype.drawNodeCollapsed = function(node, ctx, fgcolor, bgcolor) {
    ...
    if (shape == "circle") {
      ...
      ctx.beginPath();
      ctx.roundRect(
        node.size[0] * 0.5 - collapsed_radius,
        node.size[1] * 0.5 - collapsed_radius,
        2 * collapsed_radius,
        2 * collapsed_radius,
        5
      );
      ...
      ctx.beginPath();
      ctx.roundRect(
        node.size[0] * 0.5 - collapsed_radius * 0.5,
        node.size[1] * 0.5 - collapsed_radius * 0.5,
        collapsed_radius,
        collapsed_radius,
        2
      );
...
*/
  ...
  LGraphCanvas.onShowMenuNodeProperties = function(value, options, e, prev_menu, node) {
    ...
    var menu = new LiteGraph.ContextMenu(
      entries,
      { event: e, callback: inner_clicked, parentMenu: prev_menu, allow_html: true, node: node },
      ref_window
    );
  ...
  LGraphCanvas.onShowTitleEditor = function(value, options, e, menu, node) {
    ...
    dialog.innerHTML =
      "<span class='name'>Title</span><input autofocus type='text' class='value'/><button>OK</button>";
  ...
  LGraphCanvas.prototype.showEditPropertyValue = function(node, property, options) {
    ...
    dialog.className = "graphdialog";
    dialog.innerHTML =
      "<span class='name'>" + property + "</span>" + input_html + "<button>OK</button>";
  ...
  LGraphCanvas.onMenuNodeMode = function(value, options, e, menu, node) {
    new LiteGraph.ContextMenu(["Always", "On Event", "Never"], {
      event: e,
      callback: inner_clicked,
      parentMenu: prev_menu,
      node: node
  ...
  LGraphCanvas.onMenuNodeColors = function(value, options, e, menu, node) {
    ...
    for (var i in LGraphCanvas.node_colors) {
      ...
      var value = {
        value: i,
        content: "<span style='display: block; color:" +
          color.color +
          "; background-color:" +
          color.bgcolor +
          "'>" +
  ...
  LGraphCanvas.onMenuNodeShapes = function(value, options, e, menu, node) {
    ...
    new LiteGraph.ContextMenu(["box", "round"], {
      event: e,
      callback: inner_clicked,
      parentMenu: menu,
      node: node
  ...
  LGraphCanvas.prototype.processContextMenu = function(node, event) {
    ...
    if (slot) {
      ...
      if (slot.input && slot.input.type == LiteGraph.EVENT) options.title = "Event";
    ...
    function inner_option_clicked(v, options, e) {
      ...
      if (v == slot) {
        if (v.input) node.removeInput(slot.slot);
        else if (v.output) node.removeOutput(slot.slot);
        return;
      }
...
*/
  ...
  LiteGraph.closeAllContextMenus = function(ref_window) {
    ...
    for (var i in result) {
      ...
      else if (result[i].parentNode) result[i].parentNode.removeChild(result[i]);
    }
  };
  ...
  LiteGraph.extendClass = function(target, origin) {
    ...
    if (
      origin.prototype //copy prototype properties
    )
      for (var i in origin.prototype) {
        ...
        if (
          target.prototype.hasOwnProperty(i) //avoid overwritting existing ones
        )
          continue;
